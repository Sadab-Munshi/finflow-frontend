...
async function getAISummary(
  ...
  try {
    ...
    console.error('[PROCESS QUEUE] AI summary error:', e)
    return null
  }
}
...
function generatePDF(params: {
  ...
  // ── AI FINANCIAL SUMMARY (structured 4-section format) ──
  if (aiSummary) {
    need(40)
    pdf.setFontSize(14)
    pdf.setFont('helvetica', 'bold')
  ...
  // ── KEY METRICS BOXES (distinct colors per box) ──
  need(50)
  y += 4
  ...
  // ── SECTION 1: INCOME BREAKDOWN BY SOURCE ──
...
async function sendEmailWithPDF(params: {
  ...
  console.log(`[PROCESS QUEUE] Sending email+PDF to: ${to}`)
  ...
  const data = await response.json()
  console.log('[PROCESS QUEUE] Brevo response:', JSON.stringify(data))
  return data
}
...
async function processUserReport(queueItem: QueueItem) {
  ...
  // Build the report date from month name + year
  const monthNamesList = ['January','February','March','April','May','June','July','August','September','October','November','December']
  const monthIdx = monthNamesList.indexOf(month)
  if (monthIdx < 0) throw new Error(`Invalid month name: ${month}`)

  const reportDate = new Date(year, monthIdx, 1)
  ...
  if (txError) throw new Error('Transactions fetch failed')
  ...
  if (monthTx.length === 0) {
    console.log(`[PROCESS QUEUE] No transactions for ${user.email} in ${prevMonthName}`)
  ...
  const fileName = `${user_id}/${reportYear}-${reportMonth}.pdf`
  ...
  if (uploadError) {
    console.error(`[PROCESS QUEUE] Storage upload failed for ${user_id}:`, uploadError)
  } else {
    ...
    if (urlData?.signedUrl) {
      ...
      console.log(`[PROCESS QUEUE] Report saved to storage for ${user_id}`)
    }
  }
  ...
  // 9. Send browser push notification
  ...
  try {
    ...
    console.error(`[PROCESS QUEUE] Push notification error for ${user_id}:`, pushErr)
  }

  console.log(`[PROCESS QUEUE] Completed for: ${user.email}`)
}
...
export async function POST(request: NextRequest) {
  ...
  const results = await Promise.allSettled(
    batch.map(item => processUserReport(item as QueueItem))
  )
