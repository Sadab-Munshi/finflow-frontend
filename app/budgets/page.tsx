    ...
    <Layout>
      <div className="max-w-2xl mx-auto pb-8">
        ...
        {filtered.length > 0 && (
          ...
          >
            {/* Remaining headline */}
            <p className="text-xs font-medium opacity-75 mb-1">Remaining this month</p>
            <p className="text-4xl font-bold tracking-tight leading-none mb-4">
              {formatIndianCurrency(Math.abs(totalRemaining))}
              <span className="text-lg font-normal opacity-70 ml-2">
                of {formatIndianCurrency(totalBudget)}
              </span>
            </p>

            {/* Stat pills */}
            <div className="flex gap-3 mb-4">
              <div className="flex-1 rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.15)' }}>
                <p className="text-[11px] opacity-70 mb-0.5">Spent</p>
                <p className="text-base font-semibold">{formatIndianCurrency(totalSpent)}</p>
              </div>
              <div className="flex-1 rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.15)' }}>
                <p className="text-[11px] opacity-70 mb-0.5">Budgets</p>
                <p className="text-base font-semibold">{filtered.length} active</p>
              </div>
            </div>
